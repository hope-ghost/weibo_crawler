...
logging_path = get_resource_path("logging.conf")
if not os.path.isfile(logging_path):
    # 如果logging.conf不存在，尝试使用默认配置
...
def get_safe_encoding():
    """安全地获取系统编码，兼容打包成exe后的情况"""
    try:
        # 检查 sys.stdout 是否存在且有效
...
def _sleep_with_stop_check(seconds):
    """分片sleep，期间检查停止标志
    ...
    try:
        import weibo_patch
        check_stop = weibo_patch.get_stop_flag
    except:
        # 如果无法导入weibo_patch，使用普通sleep
        sleep(seconds)
        return
...
class Weibo(object):
    def __init__(self, config):
        ...
        if isinstance(since_date, int):
            ...
            sys.exit()
        self.since_date = since_date  # 起始时间，即爬取发布日期从该值到结束日期的微博，形式为yyyy-mm-ddThh:mm:ss，如：2023-08-21T09:23:03
        
        # 处理结束日期（end_date），如果不提供则默认为现在
        end_date_str = config.get("end_date", "")
        if end_date_str:
            if self.is_date(end_date_str):
                self.end_date = "{}T23:59:59".format(end_date_str)  # 设置为当天的最后一秒
            elif self.is_datetime(end_date_str):
                self.end_date = end_date_str
            else:
                logger.warning(f"end_date 格式不正确: {end_date_str}，将使用当前时间作为结束时间")
                self.end_date = datetime.now().strftime(DTFORMAT)
        else:
            # 如果没有提供end_date，使用当前时间
            self.end_date = datetime.now().strftime(DTFORMAT)
        ...
        core_cookies = {}   # 核心包
        backup_cookies = {} # 备份
        all_cookies = {}    # 完整Cookie
        
        # Cookie处理：优先尝试使用完整Cookie，如果失败再尝试精简版本
        if cookie_string:
            # 首先解析所有Cookie
            for pair in cookie_string.split(';'):
                if '=' in pair:
                    ...
                    # 提取核心SUB
                    if key == 'SUB':
                        core_cookies['SUB'] = value
                    
                    # 提取备份指纹
                    if key in ['_T_WM', 'XSRF-TOKEN', 'WBPSESS']:
                        backup_cookies[key] = value
        
        # 如果没有解析到完整Cookie，尝试使用正则表达式提取（兼容旧格式）
        if not all_cookies and cookie_string and "SUB=" in cookie_string:
            # 1. 提取核心 SUB
        ...
        if not all_cookies and not core_cookies and cookie_string:
            for pair in cookie_string.split(';'):
                if '=' in pair:
                    key, value = pair.split('=', 1)
                    key = key.strip()
                    value = value.strip()
                    all_cookies[key] = value.strip()
                    if key == 'SUB':
                        core_cookies['SUB'] = value
        ...
        # 优先使用完整Cookie，如果没有完整Cookie，则使用核心Cookie
        if all_cookies:
            requests_session.cookies.update(all_cookies)
            logger.info(f"已加载完整Cookie，共 {len(all_cookies)} 个字段")
        elif core_cookies:
            requests_session.cookies.update(core_cookies)
            logger.info(f"已加载核心Cookie（SUB）")
        elif cookie_string and cookie_string.strip() and cookie_string != "your cookie":
            # 如果没有提取到Cookie，但配置中有Cookie字符串，警告用户
            logger.warning(f"Cookie格式可能不正确，无法解析。请检查Cookie格式是否正确。")

        self.session = requests_session
        try:
            # Session预热：请求m.weibo.cn获取最新指纹
            self.session.get("https://m.weibo.cn", headers=self.headers, timeout=10)
            logger.info("Session 预热成功，服务器已下发最新指纹。")
            
        except Exception as e:
            # 请求失败时，如果使用的是完整Cookie，尝试使用核心Cookie
            logger.warning(f"Session 预热失败 ({e})，尝试使用精简Cookie...")
            if all_cookies:
                requests_session.cookies.clear()
                if core_cookies:
                    requests_session.cookies.update(core_cookies)
                if backup_cookies:
                    requests_session.cookies.update(backup_cookies)
        ...
        if not isinstance(user_id_list, list):
            if not os.path.isabs(user_id_list):
                user_id_list = get_resource_path(user_id_list)
            ...
            user_config_list = [
                {
                    ...
                    "end_date": self.end_date if hasattr(self, 'end_date') else datetime.now().strftime(DTFORMAT),
        ...
        while retries < max_retries:
            ...
            try:
                ...
                if 'application/json' not in content_type and 'text/html' in content_type:
                    ...
                    # 检查是否是反爬虫页面
                    if 'wbBotDetector' in response_text or 'bot' in response_text.lower() or '验证' in response_text or '验证码' in response_text or 'captcha' in response_text.lower():
                        logger.error(f"⚠️ ⚠️ ⚠️ 请求被识别为机器人或被拦截！⚠️ ⚠️ ⚠️")
                        logger.error(f"检测到反爬虫页面（包含 wbBotDetector），微博API拒绝了请求。")
                        logger.error(f"")
                        logger.error(f"【解决方案】")
                        logger.error(f"1. ✅ 检查Cookie是否有效：")
                        logger.error(f"   - 请前往 https://m.weibo.cn 登录微博")
                        logger.error(f"   - 按F12打开开发者工具 -> Network -> 任意请求 -> Headers -> 复制完整的Cookie")
                        logger.error(f"   - 确保Cookie中包含 SUB 字段（这是必需的）")
                        logger.error(f"   - 将完整Cookie粘贴到GUI的Cookie输入框中")
                        logger.error(f"")
                        logger.error(f"2. ✅ Cookie可能已过期：")
                        logger.error(f"   - Cookie通常有时效性，如果长时间未使用会过期")
                        logger.error(f"   - 建议重新获取最新的Cookie并更新配置")
                        logger.error(f"")
                        logger.error(f"3. ✅ 请求频率过高：")
                        logger.error(f"   - 建议增加延迟时间或降低爬取速度")
                        logger.error(f"   - 可以尝试减少'每页微博数'或增加爬取间隔")
                        logger.error(f"")
                        logger.error(f"4. ✅ IP或账号被限制：")
                        logger.error(f"   - 如果持续出现此问题，可能需要等待一段时间后重试")
                        logger.error(f"   - 或者尝试更换网络环境")
                        logger.error(f"")
                        logger.error(f"【当前请求信息】")
                        logger.error(f"  URL: {url}")
                        logger.error(f"  重试次数: {retries + 1}/{max_retries}")
                        logger.error(f"  响应前200字符: {response_text[:200]}")
                        
                        # 如果这是第一次重试，尝试重新预热session
                        if retries == 0:
                            logger.info("正在尝试重新预热Session并等待...")
                            try:
                                sleep(3)
                                self.session.get("https://m.weibo.cn", headers=self.headers, timeout=10)
                                sleep(2)
                                logger.info("Session重新预热完成")
                            except:
                                pass
                    else:
                        logger.warning(f"请求 {url} 收到HTML响应而不是JSON (Status: {response.status_code}, Content-Type: {content_type}). 响应前500字符: {response_text}")
                    raise ValueError("收到HTML响应而不是JSON")
                ...
                # 分片sleep，期间检查停止标志
                _sleep_with_stop_check(sleep_time)
            except ValueError as ve:
                retries += 1
                sleep_time = backoff_factor * (2 ** retries)
                # 记录更详细的错误信息
                try:
                    response_content = response.text[:200] if 'response' in locals() and hasattr(response, 'text') else "无法获取响应内容"
                    logger.error(f"JSON 解码失败，错误信息：{ve}。响应状态码: {response.status_code if 'response' in locals() else 'N/A'}，响应前200字符: {response_content}。等待 {sleep_time} 秒后重试...")
                except:
    ...
    def user_to_csv(self):
        """将爬取到的用户信息写入csv文件"""
        file_dir = os.path.join(get_base_dir(), "weibo")
    ...
    def get_user_info(self):
        ...
        while retries < max_retries:
            ...
            try:
                import weibo_patch
                if weibo_patch.get_stop_flag():
                    logger.info("检测到停止请求，停止获取用户信息")
                    return 1  # 返回1表示获取失败，这样可以跳过当前用户
            except:
                pass
            ...
            try:
                ...
                if 'application/json' not in content_type and 'text/html' in content_type:
                    ...
                    if 'wbBotDetector' in response_text or 'bot' in response_text.lower() or '验证' in response_text or '验证码' in response_text or 'captcha' in response_text.lower():
                        ...
                        logger.warning(f"收到HTML响应而不是JSON，可能被重定向到错误页面。响应前100字符: {response_text[:100]}")
                    raise ValueError("收到HTML响应而不是JSON")
                ...
                # 记录更详细的错误信息
                try:
                    response_content = response.text[:500] if 'response' in locals() and hasattr(response, 'text') else "无法获取响应内容"
                    status_code = response.status_code if 'response' in locals() and hasattr(response, 'status_code') else 'N/A'
                    content_type = response.headers.get('Content-Type', 'N/A') if 'response' in locals() and hasattr(response, 'headers') else 'N/A'
                    logger.error(f"JSON 解码失败，错误信息：{ve}。响应状态码: {status_code}，Content-Type: {content_type}，响应前500字符: {response_content}。等待 {sleep_time} 秒后重试...")
                except Exception as debug_e:
                    logger.error(f"JSON 解码失败，错误信息：{ve}。调试信息获取失败: {debug_e}。等待 {sleep_time} 秒后重试...")
                # 分片sleep，期间检查停止标志
                _sleep_with_stop_check(sleep_time)
        ...
        try:
            ...
            if success:
                ...
                with open(error_file, "ab") as f:
                    ...
                    f.write(error_entry.encode(get_safe_encoding()))
        except Exception as e:
            # 生成原始微博URL
    ...
    def download_files(self, file_type, weibo_type, wrote_count):
        try:
            ...
            if has_files:
                ...
                # 在打包成exe后，tqdm可能无法正常显示，使用普通循环并输出日志
                weibo_list = self.weibo[wrote_count:]
                total_count = len(weibo_list)
                if is_frozen():
                    # 打包环境：使用普通循环，输出进度日志
                    for idx, w in enumerate(weibo_list, 1):
                        if weibo_type == "retweet":
                            if w.get("retweet"):
                                w = w["retweet"]
                            else:
                                continue
                        if w.get(key):
                            logger.info("正在下载%s (%d/%d)", describe, idx, total_count)
                            self.handle_download(file_type, file_dir, w.get(key), w)
                else:
                    # 正常环境：使用tqdm显示进度条
                    for w in tqdm(weibo_list, desc="Download progress"):
                        if weibo_type == "retweet":
                            if w.get("retweet"):
                                w = w["retweet"]
                            else:
                ...
                for w in weibos:
                    ...
                    if w["card_type"] == 9:
                        ...
                        if wb:
                            ...
                            # 检查日期范围：需要在since_date和end_date之间
                            end_date = datetime.strptime(self.end_date, DTFORMAT) if hasattr(self, 'end_date') else datetime.now()
                            
                            # 如果微博时间晚于结束日期，跳过
                            if created_at > end_date:
                                logger.debug(f"跳过晚于结束日期 {end_date.strftime(DTFORMAT)} 的微博: {wb.get('id', 'unknown')} (发布时间: {created_at.strftime(DTFORMAT)})")
                                continue
    ...
    def get_filepath(self, type):
        ...
        try:
            ...
            # 使用get_base_dir()确保路径在可执行文件目录
            base_dir = get_base_dir()
            file_dir = os.path.join(base_dir, "weibo", dir_name)
            if type in ["img", "video", "live_photo"]:
                file_dir = os.path.join(file_dir, type)
            file_dir = ensure_dir(file_dir)
            if type in ["img", "video", "live_photo"]:
                return file_dir
            file_path = os.path.join(file_dir, str(self.user_config["user_id"]) + "." + type)
            return file_path
        except Exception as e:
            logger.exception(e)
    ...
    def export_comments_to_csv_for_current_user(self):
        ...
        try:
            ...
            formatted_rows = []
            comments_by_weibo = {}  # 用于按微博ID分组，使用原始weibo_id作为key
            for row in rows:
                formatted_row = list(row)
                original_weibo_id = formatted_row[1]  # 保存原始的weibo_id用于文件名
                # 格式化id字段（索引0）和weibo_id字段（索引1）
                if formatted_row[0] is not None:
                    formatted_row[0] = '\t' + str(formatted_row[0])
                if formatted_row[1] is not None:
                    formatted_row[1] = '\t' + str(formatted_row[1])
                formatted_rows.append(formatted_row)
                # 使用原始weibo_id进行分组，避免文件名包含制表符
                comments_by_weibo.setdefault(original_weibo_id, []).append(formatted_row)
        ...
        with open(file_path, "rb") as f:
            ...
            for line in lines:
                ...
                if len(info) > 0 and info[0].isdigit():
                    ...
                    # 添加end_date到用户配置
                    user_config["end_date"] = self.end_date if hasattr(self, 'end_date') else datetime.now().strftime(DTFORMAT)
